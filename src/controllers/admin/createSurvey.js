...
module.exports = ({ final }) => wrapAsync(async (req, res, next) => {
  ...
  try {
    ...
    const startDate = req.body.startDate ? new Date(new Date(req.body.startDate).setUTCHours(0, 0, 0, 0)) : null
    const endDate = req.body.endDate ? new Date(new Date(req.body.endDate).setUTCHours(23, 59, 59, 999)) : null
    ...
    const Survey = await db.Survey.create({
      ...
      final,
      emailsSent: (!startDate || startDate.getTime() === (d => d.setUTCHours(0, 0, 0, 0))(new Date()) && final),
